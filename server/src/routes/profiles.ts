...
// Create or update a profile. Expects { userId, name?, bio?, specialization?, location?, avatarUrl?, pricePerSession? }
router.post('/', async (req, res) => {
  const { userId, name, bio, specialization, location, avatarUrl, pricePerSession } = req.body
  if (!userId) return res.status(400).json({ error: 'userId is required' })
  try {
    const profile = await prisma.profile.upsert({
      where: { userId },
      create: { userId, name, bio, specialization, location, avatarUrl, pricePerSession: pricePerSession ? parseFloat(pricePerSession) : undefined },
      update: { name, bio, specialization, location, avatarUrl, pricePerSession: pricePerSession ? parseFloat(pricePerSession) : undefined }
    })
    res.json(profile)
  } catch (err) {
